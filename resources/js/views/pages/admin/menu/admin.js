        ...
        success: function (response) {
            ...
            $('.dd').nestable({
                ...
                contentCallback: (item) => {
                    ...
                    return `<i class="${item.icon}"></i>&nbsp;${item.title}${item.type == 0 ? ' | <span class="text-danger">separator</span>' : ''} ${item.type == 0 || item.route == null ? '' : `<a href="${item.url}" class="dd-nodrag" target="_blank">${item.route}</a>`} ${btn}`;
                }
            });
            isEdit(false)
        },
